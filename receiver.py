...
class STPReceiver:
    ...
    def handle_syn(self):
        ...
        while True:
            try:
                segment, origination = self.receive_segment()
                if segment.type == SYN:
                    self.expected_seqno = (segment.seqno + 1) % (2 ** 16 - 1)
                    self.init_seqno = (segment.seqno + 1) % (2 ** 16 - 1)
                    self.send_ack(self.expected_seqno, origination)
                    break
        ...
        with open(self.file_to_save, 'wb') as file:
            while True:
                try:
                    ...
                    if segment.type == SYN:
                        self.send_ack(self.init_seqno, origination)
                        ...
                        self.handle_fin(origination)
                        break
                except socket.timeout:
                    continue

    def handle_fin(self, origination):
        self.send_ack((self.expected_seqno + 1) %
                      (2 ** 16 - 1), origination)
